...
export function handleReorder(
  ...
  options: {
    ...
    /** Audit action name (e.g. 'server_reorder'). */
    auditAction: string;
  ...
  return (req: Request, res: Response) => {
    ...
    saveConfig(config, { action: options.auditAction, details: { order: newOrder } });
...
export function handleToggleEnabled(
  ...
  return (req: Request, res: Response) => {
    ...
    saveConfig(config, { action: options.auditAction, details: { [`${options.entityName.toLowerCase()}Id`]: id, enabled } });
    res.json({ id, enabled });
  };
}
