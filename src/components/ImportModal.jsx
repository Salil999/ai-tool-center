...
export function ImportModal({ onClose, onImport, onError }) {
  ...
  const handleCustomImport = async (e) => {
    ...
    setImporting('custom');
    try {
      const result = await importFromCustomFile(path, customKey.trim() || 'mcpServers');
      onImport(result);
      onClose();
    } catch (err) {
      setImporting(null);
    ...
    <div className="modal edit-modal">
      ...
      <div className="modal-body">
        ...
        <div className="import-custom">
          <h3 className="import-section-title">Custom JSON file</h3>
          <form onSubmit={handleCustomImport} className="import-custom-form">
            ...
            <div className="form-group">
              <label htmlFor="import-key">Config key (default: mcpServers)</label>
              <input
                type="text"
                id="import-key"
                value={customKey}
