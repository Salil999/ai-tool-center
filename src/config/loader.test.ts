...
describe('config/loader', () => {
  ...
  describe('saveConfig', () => {
    it('writes config to file', () => {
      const config = { servers: { a: { name: 'A', command: 'node' } } };
  ...
  describe('loadConfig', () => {
    it('creates default config when file does not exist', () => {
      ...
      expect(config.projectDirectories).toBeDefined();
    ...
    it('loads existing config', () => {
      const config = { servers: { x: { name: 'X', command: 'node' } } };
      saveConfig(configPath, config);
    ...
    it('merges with DEFAULT_CONFIG', () => {
      ...
      saveConfig(configPath, config);
      const loaded = loadConfig(configPath);
      expect(loaded.projectDirectories).toEqual([]);
    });
