...
function canonicalToOpenCodeMcp(servers: Record<string, import('../types.js').Server>): Record<string, unknown> {
  ...
  for (const [id, s] of Object.entries(servers)) {
    ...
    if (s.url) {
      ...
      const { command: cmd, args } = normalizeCommand(s.command, s.args || []);
...
/**
 * Project scope: write to {projectPath}/opencode.json with mcp key.
 * Project-specific, safe to check into Git.
 */
export function exportToOpenCodeProjectScope(
  servers: Record<string, import('../types.js').Server>,
  projectPath: string
): { path: string; success: boolean } {
  const projectConfigPath = path.join(resolvePath(projectPath), 'opencode.json');
  if (!isPathSafe(projectConfigPath)) {
    throw new Error('Path is not allowed');
  }
  ensureDir(projectConfigPath);
  let existing: Record<string, unknown> = {};
  try {
    const raw = readOpenCodeConfig(projectConfigPath);
    if (raw) existing = raw;
  } catch {
    /* ignore */
  }
  ...
  writeConfig(projectConfigPath, merged);
  return { path: projectConfigPath, success: true };
}
